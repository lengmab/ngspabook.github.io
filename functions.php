...
function theme_scripts() {
    ...
    wp_localize_script('theme-script', 'screenReaderText', array(
        'expand' => __('expand child menu', 'book'),
        'collapse' => __('collapse child menu', 'book'),
    ));
}
...
function theme_widgets_init2() {
    register_sidebar(array(
        'name'          => __('Primary Widget Area', 'book'),
        'id'            => 'primary',
        'description'   => __('Add widgets here to appear in your sidebar on blog posts and archive pages.', 'book'),
...
function _theme_get_excerpt($args = array()) {
	...
	$more_tag                     = isset($args['more_tag'])                     ? $args['more_tag']                     : __('Read more', 'book');
...
function theme_get_category_list() {
	return str_replace(
		'<a',
		'<a class="u-textlink"',
		get_the_category_list(_x(', ', 'Used between list items, there is a space after the comma.', 'book'))
	);
}

function theme_get_tags_list() {
    return get_the_tag_list('', _x( ', ', 'Used between list items, there is a space after the comma.', 'book'));
}
...
function getProductButtonHtml ($button_html, $product, $clickTypeProductbutton = 'add-to-cart', $contentProductbutton = '') {
    ...
    $buttonText = sprintf(__('Add to cart', 'book'));
    if ($contentProductbutton !== '') {
        $buttonText =  sprintf(__($contentProductbutton, 'book'));
    }
...
function theme_404_content($args = '') {
    $args = wp_parse_args($args, array(
        'error_title' => __('Nothing here', 'book'),
        'error_message' => __('It looks like nothing was found at this location. Maybe try a search?', 'book')
    ));
    extract($args);
...
function get_categories_filter_html($args) {
    ...
    // add item all
    $categories_filter_html .= strtr($args['itemTemplate'], array('{categories_filters_content}' => __('All', 'book'), '{categories_filters_value}' => get_permalink(wc_get_page_id('shop'))));
    // add item featured
    $featuredLink = class_exists('Woocommerce') ? add_query_arg('featured', 'true', get_permalink(wc_get_page_id('shop'))) : '';
    $item = strtr($args['itemTemplate'], array('{categories_filters_content}' => __('Featured', 'book'), '{categories_filters_value}' => $featuredLink));
